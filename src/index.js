...
client.once('ready', async () => {
  ...
  // Slash komutlarını global ve yerel (guild) olarak Discord'a kaydet
  try {
    ...
    // Anlık görünmesi için bağlı olduğu sunuculara da tek tek kaydet
    for (const [guildId, guild] of client.guilds.cache) {
      await guild.commands.set(commandsList.map(c => c.data.toJSON())).catch(err => {
        console.warn(`Guild (${guildId}) için yerel komutlar kaydedilemedi:`, err.message);
      });
    }
...
client.on('interactionCreate', async interaction => {
  ...
  // Kanal Kısıtlaması Kontrolü (/komut-kanali komutu her zaman her kanalda çalışabilir)
  if (interaction.commandName !== 'komut-kanali') {
    ...
    if (guildConfig.botChannelId && interaction.channelId !== guildConfig.botChannelId) {
      return interaction.reply({
        content: `❌ Bu botun komutlarını sadece <#${guildConfig.botChannelId}> kanalında kullanabilirsiniz!`,
        ephemeral: true
      });
    }
  }
