...
`
          ...
          {
            name: '🤖 Yapay Zeka & Ayar Komutları (5 Adet)',
            value: `
...
\`/komut-kanali [kanal]\` - Bot komutlarının kullanılacağı tek kanalı belirler (Yönetici).
`
          }
        )
  ...
  {
    data: new SlashCommandBuilder()
      .setName('komut-kanali')
      .setDescription('Bot komutlarının kullanılabileceği tek bir Discord kanalı belirler.')
      .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
      .addChannelOption(option => 
        option.setName('kanal')
          .setDescription('Komutların kısıtlanacağı metin kanalı (Boş bırakılırsa kısıtlama kaldırılır)')
          .setRequired(false)),
    async execute(interaction) {
      await interaction.deferReply({ ephemeral: true });
      ...
      if (targetChannel) {
        ...
        await interaction.editReply({
          content: `✅ Bot komutları başarıyla sadece ${targetChannel} kanalına kısıtlandı. Artık diğer kanallardan yazılan komutlar engellenecektir.`
        });
      } else {
        db.updateGuildData(interaction.guildId, {
          botChannelId: null
        ...
        await interaction.editReply({
          content: '✅ Bot komutlarındaki kanal kısıtlaması kaldırıldı. Artık komutlar tüm kanallarda kullanılabilir.'
        });
      }
    }
  }
